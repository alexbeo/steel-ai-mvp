...
**Shadow validation (Phase 2, PR S1-S5)** — retrospective проверка гипотезы «AI снижает Al ≥10% без потери качества» на `heats.db`. Spec: `docs/superpowers/specs/2026-05-20_deox-shadow-validation.md`. Для каждой исторической плавки вычисляет AI-рекомендацию Al (`EtaAlPredictor` + `compute_al_demand_slag_aware`) и сравнивает с фактическим `al_added_kg` в pure-Al-эквиваленте. Модули: `shadow_validation.py` (`run_shadow_comparison` → `ShadowComparison` + quality gate), `shadow_stats.py` (`compute_shadow_stats` → `ShadowStats`: paired Wilcoxon + bootstrap CI, `hypothesis_met` = median Δ≤−10% AND p<0.05 AND CI_high<0 AND n_quality_pass≥30, стратификация plant×method), `shadow_reporter.py` (HTML+JSON, savings kg+€, disclaimers). Endpoints: `POST /api/deox/shadow/run` + `GET /api/deox/shadow/report`; UI sub-tab «🌓 Shadow-валидация». CLI: `scripts/generate_shadow_report.py` (>2000 плавок), `scripts/smoke_test_shadow.py` (E2E S1→S2→S3 на synthetic). **Честный framing**: retrospective ≠ counterfactual; экономия от высокого прогнозного η (меньше потерь шлак/угар), НЕ недодозировка; synthetic тавтологичен (AI re-predicts свою η + residual budget → median Δ≈+95..+118%, `hypothesis_met=False`) — реальная −10% валидация только на real `heats.db`.

**Не входит в MVP**: кинетика растворения Al, баланс FeO в шлаке, комбинированное раскисление (Al+FeSi+Ca), feedback loop, интеграция с анализаторами O. Phase 3 — production closed-loop (AI-рекомендация = то, что оператор реально подаёт), change-management. Это фазы v0.6+.
