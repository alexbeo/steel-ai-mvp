...
| **6** | **Job infrastructure**: расширить `jobs.py` для long-running ops (training, NSGA-II, LLM). Реализовать polling component `job-progress.js` + lazy Plotly loader. **Также:** глобальный `@app.exception_handler(RequestValidationError)` который оборачивает payload через `_json_default` (та же логика, что и в `SafeJSONResponse`) — текущий FastAPI default handler крашит на NaN/Inf в request body со 500 (`Out of range float values are not JSON compliant`); браузер не пишет NaN в JSON, но curl/non-browser клиенты могут — поднято PR 4 review (Risks #3 backlog). | `app/api/jobs.py`, `app/api/main.py` (exception handler), `app/api/responses.py` (export `_json_default`), `app/web/static/js/components/job-progress.js`, `app/web/static/js/charts/pareto.js`, `app/tests/test_api_jobs.py`, `app/tests/test_api_validation_safe.py` | Polling работает, progress отображается, Plotly загружается только когда нужен chart. `curl -d '{"o_a_initial_ppm": NaN, ...}' /api/deox/forward` → 422 (не 500), body — валидный JSON. |
| **7** | **Tab «Дизайн сплава»** (NSGA-II, ~40 с, polled). Включает price-editor, Pareto chart, candidate table, breakdown export. | `app/api/routers/design.py`, `app/api/routers/jobs.py` (DELETE), `app/web/static/js/views/design.js`, `app/web/static/js/components/price-editor.js`, `app/tests/test_api_design.py`, `app/tests/test_api_jobs.py` (cancel) | Pareto chart соответствует Streamlit Altair визуально (interactive Plotly version); price upload работает; breakdown CSV скачивается. **Также:** реализовать `DELETE /api/jobs/{id}` endpoint + cooperative cancellation в long-running ops (NSGA-II `Future.cancel()` + опциональный cancellation flag, читаемый из inside алгоритма) — текущий PR 6 cancel в UI только останавливает client polling, worker thread продолжает работу. Для training (5 мин) и LLM (3 мин) серверная отмена критична, иначе CPU/токены тратятся впустую после user cancel. (Поднято reviewer'ом PR 6 — NIT, перенесено в scope PR 7.) |
| **8** | **Tab «Обучение»** (training, ~1-5 мин, polled). | `app/api/routers/train.py`, `app/web/static/js/views/train.js`, `app/web/static/js/charts/feature-importance.js`, `app/tests/test_api_train.py` (smoke с малым n_trials) | Полный cycle обучения через UI; new model появляется в /api/models; критик warnings + LLM observations. **Также:** реализовать cooperative cancellation polling в `train_model()` Optuna callback — читать `Job.cancellation_requested` per-trial и вызывать `study.stop()` если установлен. PR 7 cancel реализован для NSGA-II partial (`Future.cancel()` для pending only); training (~5 мин) requires real cancellation для UX (иначе CPU/процесс продолжает работу после user cancel). |
...
| **12** | **Anomaly Explainer hookup** (внутри Прогноза, появляется при OOD). | `app/api/routers/predict.py` (расширение), `app/web/static/js/views/predict.js` | Кнопка «Объяснить почему рискованно» вызывает /api/anomaly-explain как job. Реализовать wide-CI-relative-to-target-range heuristic из Streamlit:794-803 (показывать кнопку при ood ИЛИ wide_ci, не только ood). Пометить /api/predict response.prediction.lower_p/upper_p как deprecated alias of q05/q95 (planned removal). |
